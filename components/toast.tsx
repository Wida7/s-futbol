    ...
    <div className="fixed top-4 right-4 z-100 flex flex-col gap-2">
      {toasts.map((toast) => (
        <div
          ...
          className={cn(
            "relative min-w-80 max-w-sm overflow-hidden rounded border bg-card/95 px-4 py-3 shadow-[0_0_20px_rgba(var(--primary-rgb,0,180,255),0.06)] backdrop-blur-md",
        ...
        >
          ...
          <div className="flex items-start gap-3">
            <span className={cn("mt-1 h-1.5 w-1.5 shrink-0 rounded-full self-center", variantDots[toast.variant || "info"])} />
            <div className="flex-1">
              <span className="block font-bold text-2sm uppercase tracking-widest text-foreground/70">
                {toast.title}
              </span>
              {toast.description && (
                <span className="mt-0.5 block font-mono text-2sm text-foreground/35">
                  {toast.description}
                </span>
              )}
            </div>
    ...
    <div className={cn("flex flex-col gap-2", className)}>
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={cn(
            "relative min-w-65 overflow-hidden rounded border bg-card/95 px-4 py-3 backdrop-blur-md",
            variantStyles[toast.variant || "info"]
          )}
        >
          <div className="pointer-events-none absolute inset-0 bg-[repeating-linear-gradient(0deg,transparent,transparent_2px,rgba(0,0,0,0.02)_2px,rgba(0,0,0,0.02)_4px)]" />
